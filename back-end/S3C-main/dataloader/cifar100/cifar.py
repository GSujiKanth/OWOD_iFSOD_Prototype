...
class CIFAR10(VisionDataset):
    ...
    def SelectfromDefault(self, data, targets, index):
        if len(index) == 0:
            return np.empty((0, data.shape[1], data.shape[2], data.shape[3]), dtype=data.dtype), np.empty((0,), dtype=targets.dtype)

        first_class_index = index[0]
        # Find all occurrences of the first class in the original targets
        first_ind_cl = np.where(targets == first_class_index)[0]

        if len(first_ind_cl) == 0:
            print(f"Warning: Class index {first_class_index} not found in provided data/targets.") # Optional Warning
            data_tmp = np.empty((0, data.shape[1], data.shape[2], data.shape[3]), dtype=data.dtype)
            targets_tmp = np.empty((0,), dtype=targets.dtype)
            start_loop_index = 0
        else:
            data_tmp = data[first_ind_cl]
            targets_tmp = targets[first_ind_cl]
            start_loop_index = 1

        for i in index[start_loop_index:]:
            ind_cl = np.where(targets == i)[0]
            if len(ind_cl) > 0:
                data_tmp = np.vstack((data_tmp, data[ind_cl]))
                targets_tmp = np.hstack((targets_tmp, targets[ind_cl])) # Use hstack for 1D target arrays
    ...
    def NewClassSelector(self, data, targets, index):
        ...
        #print('ind_np', ind_np.shape)
        #index = ind_np.reshape((5,5))
        index = ind_np
        #print('index', index.shape, index)
        data_tmp = data[index]
        targets_tmp = targets[index]
        #print("check",  data_tmp.shape)
        #print(c)
        '''
        for i in index:
            ind_cl = i
...
if __name__ == "__main__":
    ...
    testloader = torch.utils.data.DataLoader(
        dataset=testset, batch_size=100, shuffle=False, num_workers=8, pin_memory=False)
